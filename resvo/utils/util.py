...
def get_action_all_1hot(action_all, l_action):
    action_all = list(action_all)
    n_agents = len(action_all)
    actions_1hot = np.zeros([n_agents, l_action], dtype=int)
    actions_1hot[np.arange(n_agents), action_all] = 1
...
def get_action_all_1hot_batch(list_action_all, l_action):
    n_steps = len(list_action_all)
    n_agents = len(list_action_all[0])
    matrix = np.stack(list_action_all)
