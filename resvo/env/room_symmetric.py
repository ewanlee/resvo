import numpy as np
from resvo.env import room_agent
...
class Env(object):
    ...
    def __init__(self, config_env):
        ...
        self.name = 'er'
        self.l_action = 3 # {stay, to door, to lever}
        # position: lever, start, door
        # Observe self position (1-hot)
        # other agents' positions (1-hot for each other agent) 
        # total amount of incentives given to each other agent
    ...
    def get_reward(self, actions):
        door_open = self.get_door_status(actions)
        rewards = self.calc_reward(actions=actions, given_rewards=None, 
                                   door_open=door_open)
